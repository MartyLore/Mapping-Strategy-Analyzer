...
public class RangeSelectNonPolymorph extends TestCaseTemplate {

	public RangeSelectNonPolymorph() {
	}

	public RangeSelectNonPolymorph(String db, Hierarchy hierarchy) throws Exception {
		super(db, hierarchy);
	}
	...
	public void tearDown() throws Exception {
		...
		// prepare ST
		stmt.executeUpdate(String.format("DROP INDEX IDX_ST_%2$s_%3$s ON %1$s.ST_%2$s", schemaName, className, attributeName));
		
		// prepare TPC & TPCC
		for (ClassNode node : hierarchy.classList) {
			stmt.executeUpdate(String.format("DROP INDEX IDX_TPC_%2$s_%3$s ON %1$s.TPC_%2$s", schemaName, node.className.toUpperCase(), node.attributes.get(0).attName));
			if(node.type.equals("{concrete}")) {
				stmt.executeUpdate(String.format("DROP INDEX IDX_TPCC_%2$s_%3$s ON %1$s.TPCC_%2$s", schemaName, node.className.toUpperCase(), attributeName));
			}
		}
